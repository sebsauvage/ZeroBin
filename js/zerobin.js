 ...
 * @param {string} key
 * @param {string} message
 * @return {string} encrypted string data.
 */
...
/**
 * @return {string} the current script location (without search or URL hash
 *     part) e.g. http://server.com/zero/?aaaa#bbbb --> http://server.com/zero/.
 */
function scriptLocation() {
  var scriptLocation = window.location.href
                             .substring(0, window.location.href.length -
...
/**
 * @return {string} the paste unique identifier from the URL
 *   eg. 'c05354954c49a487'.
 */
function pasteID() {
...
function setElementText(element, text) {
    ...
    if ($('div#oldienotice').is(':visible')) {
        // IE<10 does not support white-space:pre-wrap; so we have to do this
        // BIG UGLY STINKING THING.
...
function applySyntaxColoring()
{
    ...
    // Remove white padding around code box.
    $('div#cleartext').css('padding', '0');
}
 ...
 * @param {array} comments : Array of messages to display
 *    (items = array with keys ('data','meta').
 */
function displayMessages(key, comments) {
    ...
    if (comments[0].meta.expire_date)
        $('div#remainingtime').removeClass('foryoureyesonly')
                              .text('This document will expire in ' + secondsToHuman(comments[0].meta.remaining_time) + '.').show();

    if (comments[0].meta.burnafterreading) {
        $('div#remainingtime').addClass('foryoureyesonly')
                              .text('FOR YOUR EYES ONLY.  Don\'t close this window, this message can\'t be displayed again.').show();
    ...
    if (comments[0].meta.opendiscussion) {
        ...
        for (var i = 1; i < comments.length; i++) {
            ...
            // (CSS will automatically shift it right.)
            ...
            var divComment = $('<div class="comment" id="comment_' +
                               ...
                               '<button id="replyToComm' +
                               comment.meta.commentid + '">Reply</button>' +
                               '</div>');
            ...
            try {
                divComment.find('span.nickname')
                          .text(zeroDecipher(key, comment.meta.nickname));
            } catch (err) { }
            ...
            if (comment.meta.vizhash) {
                divComment.find('span.nickname')
                          .before('<img src="' + comment.meta.vizhash + '"' +
                                  ' class="vizhash" title="Anonymous avatar (Vizhash of the IP address)" />');
            }
        ...
        addClickEvent('openreplybutton',
                      function(e) { open_reply(e.target, pasteID()); });
    }
}
...
function send_comment(parentid) {
    ...
    showStatus('Sending comment...', true);
...
function send_data() {
    ...
    if (!sjcl.random.isReady()) {
        showStatus('Sending paste (Please move your mouse for more entropy)...', true);
        sjcl.random.addEventListener('seeded', function() { send_data(); });
        return;
    }

    showStatus('Sending paste...', true);
    ...
    $.post(scriptLocation(), data_to_send, 'json')
        ...
        .success(function(data) {
            if (data.status == 0) {
                ...
                var deleteUrl = scriptLocation() + '?pasteid=' + data.id +
 ...
 * @param {string} element Identifier of the element to select (id="").
 */
 ...
 * @param {boolean?} spin (optional) tell if the "spinning" animation
...
function pageKey() {
    ...
    // First, strip everything after the equal sign (=)
    // which signals end of base64 string.
    i = key.indexOf('=');
...
$(function() {
    ...
    // Display status returned by php code if any
    // (eg. Paste was properly deleted)
    ...
    if ($('div#cipherdata').text().length > 1) {
        ...
        if (window.location.hash.length == 0) {
            showError('Cannot decrypt paste: Decryption key missing in URL ' +
                      '(Did you use a redirector or an URL shortener which ' +
                      'strips part of the URL ?)');
            return;
        }
